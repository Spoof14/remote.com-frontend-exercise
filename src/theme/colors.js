import { css } from 'styled-components';

const colors = css`
  --colors-irisBlue: #624de3;
  // TODO: Find the right name for these colors
  --colors-irisBlueTransparent: #624de36a;
  --colors-irisBlueLight: #6447ff;
  --colors-shades-spindle: #b7b8eb;
  --colors-darkBlue: #00234b;
  --colors-redPink: #ff4a5a;
  --colors-cosmos: #ffdbde;
  --colors-geyser: #cbd3dc;

  --colors-bayoux: #525f7f;
  --colors-lynch: #617798;
  --colors-mischka: #e8ecee;
  --colors-catskillWhite: #eaf0f6;
  --colors-linkWater: #f4f7fc;

  --colors-blank: #ffffff;
  --colors-grey-500: #697786;
  --colors-grey-light: #eaf0f673;
`;

export default colors;
